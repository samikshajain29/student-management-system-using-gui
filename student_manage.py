...
def DisplayForm():
    ...
    display_screen.geometry("1000x600")
    ...
    LeftViewForm = Frame(display_screen, width=1200,bg="yellow",padx =30,pady =50)
    LeftViewForm.pack(side=LEFT, fill=Y)
    #mid frame for displaying students record
    MidViewForm = Frame(display_screen, width=1000)
    MidViewForm.pack(side=RIGHT)
    #label for heading
    lbl_text = Label(TopViewForm, text="Student Management System", font=('Sans-serif', 20), width=600,bg="#259fed",fg="#ffffff")
    lbl_text.pack(fill=X)
    #creating registration form in first left frame
    Label(LFrom, text="Name  ", font=("Arial", 18)).pack(side=TOP)
    Entry(LFrom,font=("Arial",18),textvariable=name).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Contact ", font=("Arial", 18)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 18),textvariable=contact).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Email ", font=("Arial", 18)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 18),textvariable=email).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Rollno ", font=("Arial", 18)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 18),textvariable=rollno).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Branch ", font=("Arial", 18)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 18),textvariable=branch).pack(side=TOP, padx=10, fill=X)
    Button(LFrom,text="Submit",font=("Arial", 18, "bold"),command=register).pack(side=TOP, padx=10,pady=5, fill=X)

    #creating search label and entry in second frame
    lbl_txtsearch = Label(LeftViewForm, text="Enter name to Search", font=('Sans-serif', 18),bg="#259fed")
    lbl_txtsearch.pack()
    #creating search entry
    search = Entry(LeftViewForm, textvariable=SEARCH, font=('Sans-serif', 18), width=2)
    search.pack(side=TOP, padx=10, fill=X)
    #creating search button
    btn_search = Button(LeftViewForm, text="Search", font=('Sans-serif',15),command=SearchRecord)
    btn_search.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating view button
    btn_view = Button(LeftViewForm, text="View All",font=('Sans-serif',15), command=DisplayData)
    btn_view.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating reset button
    btn_reset = Button(LeftViewForm, text="Reset",font=('Sans-serif',15), command=Reset)
    btn_reset.pack(side=TOP, padx=10, pady=10, fill=X)
    #creating update button
    btn_update = Button(LeftViewForm, text="Update",font=('Sans-serif',15), command=Update)
    ...
    tree.column('#4', stretch=YES, minwidth=0, width=250)
    tree.column('#5', stretch=YES, minwidth=0, width=100)
    tree.pack()
    DisplayData()
...
def Update():
    ...
    if not tree.selection():
        ...
        #applying empty validation
    elif name1=='' or con1==''or email1=='' or rol1==''or branch1=='':
        tkMessageBox.showinfo("Warning","fill the empty field!!!")
    else:
        ...
        if result == 'yes':
            ...
            tree.delete(curItem)
            cursor=conn.execute("UPDATE STUD_REGISTRATION SET STU_NAME = ? , STU_CONTACT = ?,STU_EMAIL = ?,STU_ROLLNO = ?,STU_BRANCH = ? WHERE STU_ID = ?" ,(name1,con1,email1,rol1,branch1,selecteditem[0]))
            conn.commit()
            tkMessageBox.showinfo("Message","Stored successfully")
            #refresh table data
            DisplayData()
